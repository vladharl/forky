...
/**
 * fetch() wrapper that retries on HTTP 429. Honours Retry-After when present;
 * otherwise applies exponential backoff (baseDelayMs * 4^attempt). Other
 * statuses (incl. 5xx) and network errors are NOT retried — they bubble up to
 * the caller's existing fallback logic. Aborted requests are not retried.
 */
export async function fetchWithRetryOn429(
  ...
  for (let attempt = 0; attempt < max; attempt++) {
    ...
    // 429 with retries left — figure out wait and try again.
    ...
    log("info", "retry.429", { label: opts.label ?? "fetch", attempt: attempt + 1, of: max, delayMs });
